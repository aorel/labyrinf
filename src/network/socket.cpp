...
Socket::Socket() : m_Sd(-1) {
}

Socket::Socket(int sd) : m_Sd(sd) {
}
...
std::string Socket::int2ipv4(uint32_t ip){
    char buf[128];
    snprintf(buf, sizeof(buf), "%u.%u.%u.%u", ip&0xFF, (ip&0xFF00) >> 8, (ip&0xFF0000) >> 16, (ip&0xFF000000) >> 24);
    return buf;
}

struct sockaddr_in Socket::resolve(const char* host, int port){
...
void Socket::set_non_blocked_impl(int sd, bool opt) throw (std::exception){
